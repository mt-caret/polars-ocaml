...
let create_basic_df () =
  Data_frame.create_exn
    Series.[ int "integer" [ 1 ]; float "float" [ 4. ] ]
...
let build_a_dataframe_usage n =
  let df =
    Data_frame.create_exn
      Series.[ int "integer" [  ]; float "float" [  ] ]
  in
  let single_row =
    create_basic_df ()
  in
  List.init n ~f:Fn.id |> List.iter ~f:(fun i ->
    Data_frame.vstack_exn df ~other:single_row;
    if i % 100 = 0 then (
      let (_:Data_frame.t) = Sql_context.rust_sql_context_execute_with_data_frames_exn
          ~data_frames_with_names:[(df, "data")]
          ~query:"select * from data limit 1" in
      ()
    ));
  Data_frame.as_single_chunk_par df;
  let count = Sql_context.rust_sql_context_execute_with_data_frames_exn
                ~data_frames_with_names:[(df, "data")]
                ~query:"select count(*), max(integer) from data" in
...
let%bench "one chunk at a time -- 100_000 iters" =
  build_a_dataframe_usage 100_000
let%bench "one chunk at a time -- 1_000_000 iters" =
  build_a_dataframe_usage 1_000_000
let%bench "one chunk at a time -- 10_000_000 iters" =
 build_a_dataframe_usage 10_000_000
