...
let%expect_test "Basic modify -- float" =
  ...
  Series.clear series;
  Data_frame.Expert.modify_series_at_chunk_index df ~dtype:Float64 ~series_index:0 ~chunk_index:0 ~indices_and_values:[0, 10.0; 2, 20.0] |> Result.ok_or_failwith;
...
let%expect_test "Basic modify -- float option" =
  ...
  Series.clear series;
  [%expect {| 2 |}];
  Data_frame.Expert.modify_optional_series_at_chunk_index df ~dtype:Float64
    ~series_index:1 ~chunk_index:0
  ...
  |> Series.Expert.compute_null_count |> Int.to_string |> print_endline;
  [%expect {| 3 |}]
;;
