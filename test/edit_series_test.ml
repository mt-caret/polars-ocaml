...
module Edit_series = struct
  ...
  let sexp_of_t (Args {data_type; initial_values; indices_and_values; value_kind; _}) =
    ...
    | Optional ->
      [%message (T data_type: Data_type.Typed.packed) (initial_values : Value.t option list) (indices_and_values: (int * Value.t option) list)]
    | Non_null ->
      [%message (T data_type: Data_type.Typed.packed) (initial_values : Value.t list) (indices_and_values: (int * Value.t) list)]
  ;;

  let lists_generator value_generator =
    let open Quickcheck.Generator.Let_syntax in
    let%bind initial_values = Quickcheck.Generator.list value_generator in
    let index_generator = Quickcheck.Generator.map Int.quickcheck_generator ~f:(fun i -> i % List.length initial_values) in
    let index_and_value_generator = Quickcheck.Generator.tuple2 index_generator value_generator in
  ...
  let quickcheck_generator_optional=
    ...
    let (module Value) = Data_type_quickcheck.data_type_value data_type in
    let%map initial_values, indices_and_values = lists_generator [%quickcheck.generator: Value.t option] in
  ...
  let quickcheck_generator_not_null =
    ...
    let%bind (T data_type) = Data_type.Typed.quickcheck_generator_packed in
    let (module Value) = Data_type_quickcheck.data_type_value data_type in
    let%map initial_values, indices_and_values = lists_generator [%quickcheck.generator: Value.t] in
  ...
  let quickcheck_shrinker =
    Quickcheck.Shrinker.create (fun (Args {data_type; initial_values; indices_and_values; value_kind}) ->
      let (module Value) = Data_type_quickcheck.data_type_value data_type in
      match value_kind with
      | Optional ->
        let module T = (struct
        ...
        let%map.Sequence { initial_values; indices_and_values } =
          Quickcheck.Shrinker.shrink T.quickcheck_shrinker { T.initial_values; indices_and_values }
        in
...
let%expect_test "" =
  ...
  Base_quickcheck.Test.run_exn
    (module Edit_series)
    ~f:(fun (Edit_series.Args {
      data_type; initial_values; indices_and_values;
      value_kind}) ->
      let try_modify () =
        match value_kind with
        | Optional ->
          let series = Series.createo data_type "test" initial_values in
          Series.Expert.modify_optional_at_chunk_index series ~dtype:data_type ~chunk_index:0 ~indices_and_values |> Result.ok_or_failwith
        | Non_null ->
          let series = Series.create data_type "test" initial_values in
          Series.Expert.modify_at_chunk_index series ~dtype:data_type ~chunk_index:0 ~indices_and_values |> Result.ok_or_failwith
      in
      let expect_fail thunk =
        match Or_error.try_with thunk with
        | Ok () -> raise_s [%message "Expected unsupported datatype to fail: " (T data_type: Data_type.Typed.packed)]
        | Error _ -> ()
      in
...
let%expect_test "Error handling" =
  ...
  Series.Expert.modify_at_chunk_index series ~dtype:Int64 ~chunk_index:0 ~indices_and_values:[0, 0; 2, 20] |> print_error;
  [%expect
    {|
    modify_chunk_at_index: unable to downcast to type T |}];
  let series = Series.int "int" [0; 1; 2] in
  Series.Expert.modify_at_chunk_index series ~dtype:Float64 ~chunk_index:0 ~indices_and_values:[0, 1.0; 2, 20.0] |> print_error;
  [%expect
    {|
      modify_chunk_at_index: unable to downcast to type T |}];
  let series = Series.float "float" [0.0; 1.0; 2.0] in
  Series.Expert.modify_at_chunk_index series ~dtype:Float64 ~chunk_index:0 ~indices_and_values:[3, 20.0] |> print_error;
  [%expect
    {|
  ...
  Series.Expert.modify_at_chunk_index series ~dtype:Float64 ~chunk_index:0 ~indices_and_values:[-1, 20.0] |> print_error;
  [%expect
    {|
        modify_at_chunk_index_exn raised an exception. Usually this happens when accessing an index out of bounds of the chunk or passing in a value outside of the domain of dtype: (Failure
  ...
  Series.Expert.modify_at_chunk_index series ~dtype:UInt8 ~chunk_index:0 ~indices_and_values:[0, -1] |> print_error;
  [%expect
    {|
    modify_at_chunk_index_exn raised an exception. Usually this happens when accessing an index out of bounds of the chunk or passing in a value outside of the domain of dtype: (Failure
